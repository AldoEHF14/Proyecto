...
loop:
    # Convertir el valor de t0 a ASCII
    addi a0, t0, 48   # Convertir el valor de t0 a su carácter ASCII ('0' = 48 en ASCII)

    # Imprimir el valor de t0
    mv a1, a0         # Mover el valor ASCII de a0 a a1
    addi sp, sp, -16  # Reservar espacio en la pila (16 bytes)
    sb a1, 0(sp)      # Almacenar el carácter ASCII en la pila
    mv a1, sp         # Mover el puntero de la pila a a1 (para pasarlo a la syscall)
